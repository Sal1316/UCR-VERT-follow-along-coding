...
module.exports = {
  ...
  async addVideoResponse(req, res) {
    try {
      const video = await Video.findOneAndUpdate(
        { _id: req.params.videoId }, // first operator is the filter.
        { $addToSet: { responses: req.body } }, // adds item to the array if it doesnt already exist in the collection
        { runValidators: true, new: true } // get back the new values after update has happend. runValidators: true makes sure that if you have any required validations are used.
      );
  ...
  async removeVideoResponse(req, res) {
    try {
      const video = await Video.findOneAndUpdate(
        { _id: req.params.videoId },
        { $pull: { responses: { _id: req.params.responseId } } }, // means pull out from responses array any document matching the Id.
        { runValidators: true, new: true }
      );
