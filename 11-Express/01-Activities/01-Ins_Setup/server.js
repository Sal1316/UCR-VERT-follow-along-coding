...
/* Import built-in Node.js package 'path' to resolve path of files that are located 
   on the server Path, can be used to construct an absulute path of two relative 
   paths by combining __dirname and 'public/routes.html' 
   for example.*/ 
   
const app = express(); // Initialize an instance of Express.js

app.use(express.static('public'));// with this code, we are now ready to serve static files from the p

const PORT = 3001; // Specify on which port the Express.js server will run, 3001 is usually the default public folder to the client.
...
/* 
  ...
  - res.send() is used to send a response back to the client. In this case, it sends the string 
   'Navigate to /send or /routes' as the response, but its not showing up anywhere. Yes it does. Its in the homepage.
*/
...
/* 
- what this is saying is that when client enters the subdirctory '/send,' 
  we execute the GET request and send a file('sendFile.html') to the client
- .join just adds to the path url.
- whats '__dirname'? 
  ans: __dirname is equal to > C:\bootcamp\UCR-VIRT-Follow-along-coding\11-Express\01-Activities\01-Ins_Setup. 
    Looks like its the file that was scoped with the terminal.
    + dirname is a predifined global variable in Node.js that represents the directory 
      name of the current module(JavaScript file). 
...
- what are the (req, res) parameters for?
