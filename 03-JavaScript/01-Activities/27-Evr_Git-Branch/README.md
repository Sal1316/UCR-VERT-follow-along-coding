# Summarized Steps To Creating A New Feature Branch (Locally- not on github):

- make a new folder mkdir git-branch-demo
- cd to that folder
- touch README.md
* open in VSCode
- git add -A
...
* run git branchto see branches
* gis status workd the same too
* cd to new branch
* touch index.js
* add some text to index.js file
* git add -A 
* git commit -m "add index.js file"
* git checkout main 
* ls to check 
* run a merg comand ( on main run git merge feature/create-js-fiel). You want to start on the branch that you want to merge the chages to. 
* git log -2 (last 2 commits) runs a history of last 2 commits
* once feature is in your main, delete existing branch (git branch -d feature/create-js-file)
